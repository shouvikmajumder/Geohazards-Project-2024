...
def array2stream(array,dict):
    '''
    function to convert a numpy array and a dictionary of stats into a stream
    array = numpy.array that is shape = ([nSamp*nPack,nCh])
    dict = dictionary in style output from rawHeader2dict function
    ####
    How to use:
    param = {'network':'PSU', 'station':'--','fs':250,'dt':0.004, 'start':stime,
    ...
    fs = int(dict['fs'])
    dt = float(dict['dt'])
    start = obspy.UTCDateTime(str(dict['start']))
    dx = np.round(float(dict['dx']),2)
    gl = float(dict['gl'])
    units = str(dict['units'])
    # i0 = int(dict['first_ch_in_file'])
    chlist = dict['chlist']

    for i,row in enumerate(array):
        trace = obspy.Trace(data=row)
        trace.stats.network = dict['network']
        trace.stats.station = dict['station']
        trace.stats.channel = str('%04d' % (chlist[i]))
        # trace.stats.channel = 'DAS'
        trace.stats.sampling_rate = fs
        trace.stats.starttime = start
        trace.stats.distance = dx*(chlist[i])
        trace.stats.gaugelength = gl
        if units == 'nanostrain/s':
            trace.data = trace.data/8192*116*fs/gl
...
def stream2array(stream):
    ...
    '''    
    # HERE STREAM IS self.data2
    print(f"PRINT STATEMENT ------->{len(stream)}")
    print(f"PRINT STATEMENT ------->{type(stream)}")
...
def foresee_calibration():
    bad_traces = list(range(0, 23)) \
                 ...
                 + list(range(2365, 2432))
    all_traces = np.arange(0,2432)
    good_traces = np.delete(all_traces, bad_traces)
    return good_traces
